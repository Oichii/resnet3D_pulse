...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (net_input, target) in enumerate(train_loader):
        ...
        # target = torch.mean(target, 1).unsqueeze(dim=1)
...
def validate(val_loader, model, criterion):
    ...
    for i, (net_input, target) in enumerate(val_loader):
        ...
        # target = target.squeeze()
        target = target.cuda(non_blocking=True)
        # target = torch.mean(target, dim=1).unsqueeze(dim=1).cuda(non_blocking=True)
